...
export default class AppPreferences {
    static async saveAccessToken(token, secret = 'fmstyle') {
    ...
    // save message error
    static async setMessageError(value) {
        await AsyncStorage.setItem('message_error', value);
    }
    static async getMessageError() {
        return await AsyncStorage.getItem('message_error');
    }
}
